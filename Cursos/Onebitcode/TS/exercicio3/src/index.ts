function sendSpaceship(name:string, captain:string) {
    ...
    alert(`A nave ${spaceship.name} comandado pelo captão ${spaceship.captain} foi enviada em uma missão!`)
...
function accelerate(targetSpeed:number, spaceship: { name:string, speed:number }) {
    if (spaceship.speed > targetSpeed) {
        alert(`Reduzindo a velocidadde da ${spaceship.name} para ${targetSpeed}...`)
